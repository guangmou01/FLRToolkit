# Path: "server/utils/BiGauss_LOOCV_helper.R"
#
# This is a helper function to run Bi-Gaussianized calibration using
...
BiGauss_LOOCV_regularized <- function(df,
                                      id1_col, id2_col,
                                      score_col,
  ...
  out_df$leave_out_key <- mapply(make_leave_out_key,
                                 id1_chr, id2_chr, USE.NAMES = FALSE)
  ...
  # 3. Prepare outputs
  n <- nrow(out_df)
  quasi_score <- rep(NA_real_, n)
  cllr_target <- rep(NA_real_, n)
  sigma2_target <- rep(NA_real_, n)
  ...
  shiny::withProgress(message = "Calibrating by Leave-out Keys:", value = 0, {
    ...
    for (k_i in seq_along(unique_keys)) {
      ...
      quasi_score[test_idx] <- as.vector(test_scores %*% beta + alpha)
      
      calibrated_lnLR[test_idx] <- as.numeric(
        biGaussian_calibrator(
          uncal_score = test_scores,
          model = model,
          grid_k = grid_k,
          grid_len = grid_len
        )
      ...
      cllr_target[test_idx] <- model$cllr_target
      sigma2_target[test_idx] <- model$sigma2_target
  ...
  out_df$quasi_score <- quasi_score
  out_df$cllr_target <- cllr_target
  out_df$sigma2_target <- sigma2_target
...
BiGauss_LOOCV_robust <- function(df,
                                 id1_col, id2_col,
                                 score_col,
                                 score_scale = c("ln(LR)", "log10(LR)", "Raw"),
                                 prior = 0.5,
