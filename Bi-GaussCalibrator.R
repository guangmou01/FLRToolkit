...
source("func/biGaussian_robust.R")
source("func/biGaussian_regularized.R")
...
server <- function(input, output, session){
  ...
  observeEvent(input$single_run_calibration, {
    ...
    calibrated_lnLR <- calibration_result$calibrated_lnLR
    calibrated_LR <- exp(calibrated_lnLR)
    calibrated_log10LR <- calibrated_lnLR / log(10)
    
    fusion_w <- calibration_result$fusion_w
    d <- length(score_cols)
    beta <- fusion_w[1:d]
    alpha <- fusion_w[d + 1]
    ...
    result_text <- paste0(
      ...
      "Parameters of the Bi-Gaussianized Step:\n",
      "   - Target Cllr: ", round(calibration_result$Cllr, 6), "\n",
      "   - Target Sigma²: ", round(calibration_result$sigma2_target, 6), "\n"
    )
  ...
  observeEvent(input$hv_run_calibration, {
    ...
    calibrated_lnLR <- biGaussian_calibrator(
      uncal_score = val_lnLR,
      model = model,
      grid_k = input$hv_grid_k,
      grid_len = input$hv_grid_len
    )
    calibrated_LR <- exp(calibrated_lnLR)
    calibrated_log10LR <- calibrated_lnLR / log(10)
